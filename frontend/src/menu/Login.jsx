...
export default function Login() {
  ...
  // --- Handle token from Google redirect (if backend redirects back with ?token=...)
  useEffect(() => {
    ...
    const handleToken = async () => {
      setLoading(true);
      try {
        // save token & axios header
        setAuthToken(token);

        // fetch profile to store role/id
        ...
        // notify app that auth changed (Navbar listens for this)
        try { window.dispatchEvent(new Event("authchange")); } catch {}

        // navigate to feed (replace history)
        navigate("/feed", { replace: true });
      } catch (err) {
        console.error("OAuth login flow error:", err);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      try {
        ...
        // ignore profile fetch error — user is still logged in, but role may be missing
      }
